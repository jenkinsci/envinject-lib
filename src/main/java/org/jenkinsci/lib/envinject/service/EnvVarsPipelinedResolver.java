...
public class EnvVarsPipelinedResolver implements Serializable {
    ...
    public Map<String, String> getPollingEnvVars(Job<?,?> project, /*can be null*/ Node node) throws EnvInjectException {
        ...
        //                return getEnVars((Job<?,?>) lastBuild);
        //            }
        //        }
    ...
    public Map<String, String> getEnVars(Job<?,?> build) throws EnvInjectException {

        if (build == null) {
            throw new NullPointerException("A build object must be set.");
        }

        EnvInjectActionPipelinedRetriever envInjectActionRetriever = new EnvInjectActionPipelinedRetriever();
        ...
        //        }
        //        //-- End check
        // FIXME process last node if applicable
        return getFallBackMasterNode(build);

        //Get envVars from the node of the last build
        //return getDefaultEnvVarsJob(build.getProject(), builtOn);
    }

    private Map<String, String> getFallBackMasterNode(Job<?,?> project) throws EnvInjectException {
    ...
    public String resolveEnvVars(Job<?,?> build, String value) throws EnvInjectException {
    ...
    private Map<String, String> getDefaultEnvVarsJob(Job<?,?> project, Node node) throws EnvInjectException {
        assert project != null;
        assert node != null;
    ...
    private Map<String, String> gatherEnvVarsMaster(Job<?,?> project) throws EnvInjectException {
    ...
    private Map<String, String> gatherEnvVarsNode(@Nonnull Job<?,?> project, @Nonnull Node node) throws EnvInjectException {
        assert project != null;
        assert node != null;
        ...
        try {
            ...
            // TODO figure out what to do w/ WORKSPACE
